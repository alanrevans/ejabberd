%%%-------------------------------------------------------------------
%%% File    : mod_email_roster_ldap.erl
...
-define(USER_CACHE, email_roster_ldap_user_cache).
-define(GROUP_CACHE, email_roster_ldap_group_cache).
...
get_user_displayed_groups({User, Host}) ->
    Email = mod_admin_extra:get_vcard(User, Host, <<"EMAIL">>, <<"USERID">>),
...
extract_members(State, Extractor, AuthChecker, #eldap_entry{attributes = Attrs}, {DescAcc, JIDsAcc}) ->
    Host = State#state.host,
    {value,{<<"mail">>,Emails}} = lists:keysearch(<<"mail">>, 1, Attrs),
    Same_email_domain = lists:any(fun(Email) -> 
                                      [_U, D] = binary:split(Email, <<"@">>),
                                      D == DescAcc
                                      end, 
                        Emails),
    case {eldap_utils:get_ldap_attr(State#state.group_attr, Attrs),
        ...
        {ID, _Desc, {value, {GroupMemberAttr, Members}}} when ID /= <<"">>,
